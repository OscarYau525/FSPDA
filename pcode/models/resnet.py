...
class ResNet_imagenet(ResNetBase):
    ...
    def _eval_layers(self):
        ...
        for _, module in self.named_modules():
            if isinstance(module, nn.BatchNorm2d) or isinstance(module, nn.ReLU) or isinstance(module, nn.AvgPool2d) or isinstance(module, nn.MaxPool2d):
                module.eval()
...
def resnet(conf):
    ...
    if "femnist" in conf.data:
        model = ResNet_mnist(dataset=dataset, resnet_size=resnet_size)
    elif "cifar" in conf.data or "svhn" in conf.data:
...
def count_parameters(resnet_size):
    model = ResNet_imagenet(dataset="imagenet", resnet_size=resnet_size)
